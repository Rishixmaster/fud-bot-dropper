...
def simple_protect(input_apk: str, output_apk: str) -> bool:
    ensure_dirs()
    ...
    try:
        ...
        # Also update any references in manifest (like authorities)
        manifest = manifest.replace(old_pkg, new_pkg)

        with open(manifest_path, 'w', encoding='utf-8') as f:
            f.write(manifest)

        # Rename smali folders to match new package
...
async def handle_doc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    success = False
    try:
        success = simple_protect(fin, fout)
    except Exception as e:
